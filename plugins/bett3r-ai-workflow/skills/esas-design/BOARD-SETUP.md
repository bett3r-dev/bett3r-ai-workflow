...
- **It returns `ESAS_DIR_MISSING`** — the server is running and answering about *this* checkout, which simply has no `.esas/`. In a fleet worktree that is **the correct answer, not a fault to fix** (see the skill's main-checkout rule). In the main checkout it means the extractor has not run here yet: run the repo's `designTooling.extract` from `.esas.config.json` (`yarn esas` in teselly), then re-check. Do not add `ESAS_REPO_PATH` to point it elsewhere — that is how a worktree ends up writing into another checkout's design layer.

  **A repo with no graph at all can still get a map (ESAS-174).** `ESAS_DIR_MISSING` in a repo the extractor has genuinely never run in (not a worktree — the main checkout, or any repo that never adopted an ESAS extractor) does not by itself rule out `design-map select`'s `board-candidate` row: the table's rows 0-8 are about MCP capability and board reachability, not about a graph existing. `start_map_session` still runs, but it only creates a bare `.esas/` and launches nothing — neither it nor `design-map` ever starts a board (design-map D3). The owner launches the board by acting on the launch line, with `esas-session-server --non-anchor`: the flag that serves the board with no extracted reality (`graph.json` and `diagnostics.json` 404), rather than the graph-bearing launch this file documents elsewhere. Verified at esas @ de920db: `packages/esas-session-server/src/launcher.ts:261-262` parses `--non-anchor` into `args.nonAnchor = true`, and `:303` sets `anchored: !args.nonAnchor`. The launch-line wording itself is the design-map skill's, not this file's.
