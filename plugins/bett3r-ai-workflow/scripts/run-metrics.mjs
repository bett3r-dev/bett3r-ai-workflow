...
function collectRun(branch, sessionFiles) {
  const runs = [], marks = []
  const sessions = []
  const cwdVotes = new Map(); let droppedDetached = 0 // a detached pool worktree's worker (R7)
  ...
  for (const parentFile of sessionFiles) {
    ...
    for (const f of readdirSync(subDir)) {
      ...
      if (subBranch !== null && subBranch !== branch) { if (subBranch === 'HEAD') droppedDetached++; continue }
      if (subBranch === null) {
...
/**
 * The `usage` blocks of build-summary.md, as YAML — the numbers /verify-build
 * writes before it opens the PR. An agent cannot observe its own usage, so these
 * are generated here and copied verbatim, never hand-written.
 *
 ...
 *    way `summarize` prorates a phase. Slice-role dispatches are excluded so no
 *    token is counted in two blocks. Measured mid-command, it covers
 *    `/verify-build` up to the moment this runs.
 *  - A cell: tokens = input + cache write + cache read + output (the ledger's
 *    sum); activeMs = the runs' activeMs summed; model and effort = the value
 *    whose runs consumed the most tokens (a run credits all its tokens to its own
 *    dominant model/effort; a tie keeps the first seen), `null` when unknown.
 *    A role with no dispatch is `null` — never zero, never estimated.
 *
 * The last line is a YAML comment carrying the verdict (ADR-004):
 * `# RUN-METRICS-USAGE:v1 outcome=ok …` here, `outcome=error reason=…` in main().
 * `droppedDetached=<n>` on it counts subagents `collectRun` skipped because
 * their dominant gitBranch is `HEAD` — workers in a detached pool worktree,
 * whose usage is in no block (R7). It is on the verdict line only, so the
 * YAML keeps build-summary.md's shape; a fleet unit is never branch-filtered
 * and reports 0.
 */
const FRAGMENT_ROLES = { executor: 'executor', verifier: 'verifier', 'test-runner': 'testRunner' }
const TOKEN_KINDS = ['input', 'cacheWrite', 'cacheRead', 'output']

function usageCell(parts) {
  if (!parts.length) return null
  const models = new Map(), efforts = new Map()
  let tokens = 0, activeMs = 0
  for (const { r, share, active } of parts) {
    const tok = TOKEN_KINDS.reduce((t, k) => t + (share === 1 ? r.tokens[k] : Math.round(r.tokens[k] * share)), 0)
    tokens += tok
    activeMs += active
    models.set(r.model, (models.get(r.model) ?? 0) + tok)
    efforts.set(r.effort, (efforts.get(r.effort) ?? 0) + tok)
  }
  const top = (m) => {
    let best = null, bestTok = -1
    for (const [k, v] of m) if (v > bestTok) { best = k; bestTok = v }
    return best === 'unknown' ? null : best
  }
  return { model: top(models), effort: top(efforts), tokens, activeMs }
}

function usageFragment(branch, { runs, marks, droppedDetached = 0 }) {
  const yamlCell = (c) => c === null ? 'null'
    : `{ model: ${JSON.stringify(c.model)}, effort: ${JSON.stringify(c.effort)}, tokens: ${c.tokens}, activeMs: ${c.activeMs} }`
  ...
  for (const r of runs) {
    ...
    bySlice.set(key, roles)
  }

  const lines = ['# GENERATED by run-metrics --usage-fragment from the transcripts — never hand-write these numbers.']
  const ids = [...bySlice.keys()].filter(k => k !== 'unattributed').map(k => Number(k.replace(/^slice /, ''))).sort((a, b) => a - b)
  lines.push(ids.length ? 'slices:' : 'slices: []')
  for (const id of ids) {
    const roles = bySlice.get(`slice ${id}`)
    lines.push(`  - id: ${id}`, '    usage:')
    for (const role of ['executor', 'verifier', 'testRunner']) lines.push(`      ${role}: ${yamlCell(usageCell(roles[role]))}`)
  }
  const un = bySlice.get('unattributed')
  if (un) {
    lines.push('unattributed:')
    for (const role of ['executor', 'verifier', 'testRunner']) lines.push(`  ${role}: ${yamlCell(usageCell(un[role]))}`)
  } else lines.push('unattributed: null')

  const runStart = marks.length ? Math.min(marks[0].t, ...runs.map(r => r.start)) : Math.min(...runs.map(r => r.start))
  const runEnd = Math.max(...runs.map(r => r.end))
  const vbWindows = buildWindows(marks, runStart, runEnd).filter(w => w.phase === 'verify-build')
  const vbParts = []
  for (const r of runs) {
    if (FRAGMENT_ROLES[shortRole(r.agentType)]) continue
    const active = vbWindows.reduce((t, w) => t + clipTotal(r.activeIntervals, w), 0)
    if (active <= 0) continue
    vbParts.push({ r, active, share: r.activeMs > 0 ? active / r.activeMs : 0 })
  }
  const vb = vbWindows.length ? usageCell(vbParts) : null
  lines.push('verifyBuild:', `  usage: ${yamlCell(vb)}`)
  lines.push(`# RUN-METRICS-USAGE:v1 outcome=ok branch=${branch} slices=${ids.length} unattributed=${unattributedRuns} verifyBuild=${vb ? 'measured' : 'absent'} droppedDetached=${droppedDetached}`)
  return lines.join('\n')
}
...
function parseArgs(argv) {
  const o = { branch: null, emit: false, json: false, list: false, aggregate: false, agents: false, since: null, quiet: false, fleet: null, usageFragment: false }
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--usage-fragment') o.usageFragment = true
...
run-metrics — where a unit of work's time and tokens actually went
  ...
  --usage-fragment  print build-summary.md's generated usage blocks as YAML: per slice
                    (a dispatch described 'slice <id>'), per role executor | verifier |
                    testRunner, { model, effort, tokens, activeMs }; dispatches naming no
                    slice under 'unattributed'; 'verifyBuild.usage' for the /verify-build
                    window. Ends with '# RUN-METRICS-USAGE:v1 outcome=ok|error …'. Writes
                    nothing to ~/.claude/bett3r-metrics/ runs or index.
...
function main() {
  ...
  if (o.usageFragment) {
    ...
    console.log(usageFragment(reportBranch, collected))
    return
  }
