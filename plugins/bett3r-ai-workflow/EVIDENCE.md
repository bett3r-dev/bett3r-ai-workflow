...
Referenced by `/build`, `/verify-build`, `/design`, `/design-multi`, `/start-multi` and the `executor` / `verifier` / `test-runner` / `provisioner` agents. Those artifacts carry the **triggers** — the greppable condition each one must watch for. This file carries the four facts every trigger falls out of, so they are stated once instead of nine times.
