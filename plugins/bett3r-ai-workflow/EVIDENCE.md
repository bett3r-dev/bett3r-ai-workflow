...
Referenced by `/build`, `/verify-build`, `/merge-multi`, `/design`, `/design-multi`, `/start-multi`, the `full-gate` skill and the `executor` / `verifier` / `test-runner` / `provisioner` / `unit-lane` agents. Those artifacts carry the **triggers** — the greppable condition each one must watch for. This file carries the four facts every trigger falls out of, so they are stated once instead of nine times.
