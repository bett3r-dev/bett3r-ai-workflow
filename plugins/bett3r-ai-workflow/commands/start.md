...
`.work/lane.yaml` is otherwise deleted and **not** rewritten here: it is a lane's whole brief, written into a provisioned worktree from the outside, and a `/start` on your own branch is not a lane. A stale one left behind claims a deferral — of the branch-wide gate, to a run that no longer exists — and the claim is silent, which is worse than the residue. **One file means one scrub path**; two brief files means two, and a scrub can miss one.
