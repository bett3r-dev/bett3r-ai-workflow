...
**You are a phase, not a session.** Phase A ends when every `design-lane` has reported; Phase B is opened **by the owner, by hand, and no driver ever does it**; the B/C boundary ends this context the same way. Each phase opens cold and restarts from disk — Phase B from `<run>/units/`, Phase C from `<run>/subjects/` and `<run>/answers/` — so every answer must be persisted in the shape Step 5.2 reads, `map: <S>` included, **before** this context ends. The phase is read from `units[].step` and never stored.

**[`reference/design-multi-phase-boundary.md`](../reference/design-multi-phase-boundary.md) is normative** and states the rest: the per-phase restart sources, `blockedOn=awaiting-owner-sitting`, and why nothing here licenses a `/merge-multi` yield by analogy.
