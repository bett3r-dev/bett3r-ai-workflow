---
description: (experimental, v2 of /evolve) Turn a plugin repo's ai-learning issues into reviewed PRs — with a mandatory prune pass and a net-change budget, so the artifacts converge instead of accreting.
---

# /evolve-2 — issues → reviewed PRs, with pruning pressure

Process the `ai-learning` issues routed to **this** repo: prune, cluster, propose, and open PRs for review.

**This is the experimental v2.** v1 was convergent about the *backlog* and monotonic about the *artifacts* — every round could only add, and nothing was ever positioned to take anything out. `/evolve` is the only process that reads these files regularly, so **it is the only place a rule can die.** Steps 1 and 5 are what v2 adds; the rest is v1, tightened.
...
Then **report the ratio, per artifact and for the round: lines and words added vs removed, and rules added vs amended vs deleted.** This is the number that tells you whether the plugin is converging or accreting, and it is the whole reason this command has a v2. If a round adds and never removes, say so plainly in the report rather than letting it pass as progress.
...
## Principles
- **Convergent in both directions.** v1 converged the backlog; v2 also converges the artifacts. A round that only adds is a round that made every future session slightly worse.
