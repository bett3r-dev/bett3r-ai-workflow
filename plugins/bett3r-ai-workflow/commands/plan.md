...
- **An oracle must be ADEQUATE, not just green — three rules, and RED→GREEN sees none of them.** Each of these is genuinely red before the code exists and green after, so the one gate `/build` runs against every slice is structurally blind to all three.
...
Write the approved slices (the `vertical-slicing` skill's schema): `id`, `name`, `passes: false`, `depends_on`, `behavior`, `oracle` (the test that proves it), `seam` (the unit's seam it tests at — one of the top-level `seams:`, Step 3), `probe` (the one-line production mutation that must turn this slice's oracle red), `scenarios` (below — **every slice, no exceptions**, each behavioural one carrying `expected_from:` and, unless `literal`, `expected_source:`), `gates` (the project invariants the verifier must confirm), `designs` (the design node ids it delivers, when the unit has a design layer). Record the ADR path and branch. Lead each slice with behavior; `touches` (files) is an optional hint only. Add `surface` (Step 3) to every slice, and **before writing, check each one against the cap**: a slice over it without `surface.atomic:` is refused — split it, then write.
