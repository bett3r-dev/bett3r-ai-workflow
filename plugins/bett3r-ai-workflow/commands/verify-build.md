...
Two of them — **persisted-data** and **denominator-redefinition** — are worth calling out together, because they share the trait that makes them hardest to see: **the broken code is code this PR never touched.** A diff-shaped review looks straight past them. What changed is the *meaning* of something the old code still reads (a field name, a raw total), so the old code keeps compiling, keeps passing, and is now silently wrong. When a diff redefines a value's semantics, review the **readers of that value**, not just the diff.
...
- **Persisted-data / backward-compat sweep.** `"tests pass" is not evidence for a path that has no test` — a green gate is vacuous for a path the suite never runs, which is exactly where schema-evolution and backward-compat bugs live. When the diff touches **event schemas / `*.types.ts` event definitions**, **persisted collection field names**, or **idempotency/dedup records**, force three questions with *evidence*, not assertion: (1) **Schema evolution** — does a renamed/removed persisted field ship an upcaster or event-version bump? (a "rename" that touches event/aggregate-state fields is evolution — old-shaped events rehydrate to `undefined`); (2) **Backward-compat** — do rows/events persisted *before* this diff still work, and is there a test that exercises a pre-change record?; (3) **Coverage honesty** — does the changed path actually have a test, or is "green" vacuous here? Name the untested path explicitly. Any "no" is a finding. (Real miss: TV1-1911's "pure rename" renamed persisted event fields with no upcaster → aggregate rehydrated to `accounts["undefined"]`; TV1-1960's idempotent reversal keyed off an `_applied.entryId` that pre-enrichment rows lacked → silent under-reversal. Both green, both on untested paths.)
- **Denominator/threshold-redefinition sweep.** When a slice introduces a new **adjustment to a total, count, or threshold** (`effectiveTotal = total - noOpCount`, a new floor / offset / discount / skip-count), it does not just need threading to its *new* consumers — it **retroactively changes the meaning of the raw value it adjusts**. Grep every *other* site that compares against the **unadjusted** value (`< total`, `>= totalItems`, `=== count`): alerts, WARN canaries, assertions, invariant guards, cleanup policies, dashboard queries. Each is now measuring against stale semantics and is a silent false-positive/negative. Fix them in the same PR or defer explicitly with a ticket. Note this defect lives in **pre-existing, unchanged code** — it has no failing test because it was correct against the old semantics, and the new input (a no-op unit) rarely appears in happy-path fixtures. (Real miss: TV1-1979 added `effectiveTotal = totalItems - noOpCount` but left `batchProgressCleanup`'s under-count canary comparing `progressCount + errorCount < totalItems` — a false-positive WARN for *every* no-op batch, i.e. production alert-spam. Caught only incidentally, by neither the per-slice gates, the whole-PR verifier, nor any existing sweep.)
...
The sweeps above assume you can trust the word "green." Running the suite is only evidence if you read the verdict correctly, and the naive read is wrong twice over:

- **A pass/fail read from a *piped* exit code is a lie.** `yarn test 2>&1 | tail -30` reports `tail`'s exit code, not jest's — a red suite (33 failed suites / 197 failed tests, in one real case) surfaces as exit `0`, and an unqualified "tests pass" goes into the PR body, the durable record, on top of a red run — silently, in the safe-looking direction. Read the verdict from jest's own summary (`… | grep -E '^(Tests|Test Suites):|^FAIL'`) or preserve the real status (`set -o pipefail`, `${PIPESTATUS[0]}`). **If no `Tests:` summary line was parsed, the run is *inconclusive*, not green.** ("Tests pass" is not evidence for a path with no test; a laundered exit code is not even evidence that the tests passed.)
- **"All green" is the wrong bar when the base is already red.** A repo whose default suite has pre-existing failures — infra-coupled suites, a quarantine list, flaky tests — makes both "exit 0 = pass" *and* "any red = I broke it" wrong; an agent that demands green will stall or start rationalizing failures away. The sound verdict is a **baseline diff**: reusing the true base resolved at the top of this step, capture the failing-suite *set* on that base and on `HEAD` and diff the **names**. A suite that flips `PASS→FAIL` is a regression — investigate; a suite already red on the base is pre-existing — name it and move on; test-*count* jitter inside an already-red suite is infra noise, not a regression. This gives the PR body something honest to say — *"failing-suite set identical to base (N = N), zero PASS→FAIL flips"* — instead of an unqualified "tests pass."
