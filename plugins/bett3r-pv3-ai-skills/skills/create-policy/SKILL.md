...
- **Retry-eligibility is decided by HTTP status, not error type** — the outbox retries only `[ 408, 429, 502, 503, 504 ]`; a plain `Error` resolves to 500 and is a **poison event** (per-stream quarantine, admin `retry-stream` to recover). For projection lag or a dependency down throw a 503-class error (`ServiceUnavailableError`); for a terminal caller fault throw `BadRequestError`. Never a bare `Error` "so the outbox retries" — it does the opposite. See [`ddd-patterns` → DELIVERY.md](../ddd-patterns/DELIVERY.md) → "A thrown error is classified by HTTP status".
...
- **Multi-command fan-out is redelivery-restartable** — every dispatched command idempotent on replay, or progress tracked in a watermark; required regression test "redelivery after partial progress emits zero duplicate events". Full rule: [POLICIES.md → Multi-command fan-out](../ddd-patterns/POLICIES.md).
- **Soft-delete defeats `stateExists`** — a terminal command on a soft-delete aggregate needs its own guard keyed on `state?.isDeleted`, or it re-emits on redelivery. Full rule: [POLICIES.md → Multi-command fan-out](../ddd-patterns/POLICIES.md).
...
- **Non-idempotent external side-effects → reconcile by natural key, not a blind ledger** — a ledger marks *after* the effect and re-creates the act-then-mark gap; reconcile is authoritative. Full rule: [POLICIES.md → reconcile by natural key](../ddd-patterns/POLICIES.md).
- **A system-triggered policy cannot reach a user-scoped command through `executeCommand`** — it carries no principal, and `CronjobTriggered` carries no tenant. Dispatch through the client library with per-call auth headers, tenant taken from the aggregate's own record. Full rule: [POLICIES.md → Dispatch has an authorization axis](../ddd-patterns/POLICIES.md).
...
- [ ] File named `<purpose>.policy.ts`, holding **exactly one `PolicyBuilder`** — the registration name is derived from the filename, and a second builder is silently refused at boot with build, typecheck and unit tests green. Only `generate-all` proves it registered (the `route-config.json` entry).
