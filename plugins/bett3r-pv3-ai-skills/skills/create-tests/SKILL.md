...
- **For a "field X must land in Y" criterion, the input must make Y the ONLY possible source of X.** Sending `accountId` in the body and asserting it in `event.data` passes against a handler that merely spreads the body. Omit it from the body, inject it via `context.user`, then assert it still arrives. `testCommandHandler` builds `context.user` as `{ id }` only and cannot express an authenticated caller's `accountId` — that oracle needs a thin wrapper around `ports.eventsourcing.executeCommand`.
- **`MemoryDb` does not enforce PRIMARY KEY** — `createMany` assigns `coll[id]` directly; only `validateIndex` rejects. A claim table whose uniqueness guard is a derived-id PK (the `customers` precedent) has that guard **unexercised** in the memory harness: the duplicate is caught by the secondary unique index, so the test passes for the wrong reason and stays green with the real guard removed. A storage-enforced constraint is proven on `createPostgresIntegrationHarness` or not at all; the memory harness is for choreography and projection shape.
