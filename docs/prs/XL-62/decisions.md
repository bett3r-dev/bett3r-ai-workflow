...
rejected: —
supersedes: —
**Correction (post-hoc, from the orchestrator):** the original text below said `bett3r-xp-layer` "is not on this machine" as an environment-gap fact. That was wrong: the repo IS present, at `/Users/tomasruiz/Documents/development/bett3r-xp-layer`. This lane was deliberately fenced out of it by the orchestrator's own instruction (do not touch, do not import from, code to the seam only) — not by an environment gap. The orchestrator separately inspected the consumer half and confirms this unit's emitted grammar matches it exactly: `RESOLVED_BY_LITERALS = ['human', 'code', 'recommendation']`, `ResolvedByKind = 'atom' | 'neotoma' | 'human' | 'code' | 'recommendation'`, `ResolvedByCensus` gaining `recommendation: number`.

Original text, left for the record: `bett3r-xp-layer` is not on this machine, so `packages/xp-mcp/src/resolved-by.ts`, XL-24 and that repo's ADR-053 §10 could not be opened: `environment-gap: consumer repository not present locally`. The ADR states the consumer reads the line as fact, where the two source comments say explicitly that those tokens do not resolve here. The verifier judged this a nit rather than a finding — the claim is the ticket block's own premise, in-repo-cited, and asserts nothing about consumer behaviour beyond "reads the line" — but it is carried here and into the PR body rather than silently accepted.
