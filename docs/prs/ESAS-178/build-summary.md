...
verifyBuild:
  usage: null
  gate: { mode: full, verdict: FAIL, failed: [version-bump], skipped: [], inconclusive: [] }
  coherence: { critical: 0, medium: 1, low: 0, shippedUnresolved: [structureVersion-mismatch-with-esas] }
  fixSlicesAdded: 0
  adrs: []
...
Usage not measured: no-transcripts (run-metrics found no transcripts for this branch; the lane runs under the orchestrator session).

verify-build: the gate mirror has no --fast mode, so the full mirror ran: 16/17 PASS, version-bump FAIL by directive (no plugin.json bump in a unit). It first caught skills/design-map/SKILL.md description at 1128 chars (over the ~1024 cap), fixed inline before the PR.
